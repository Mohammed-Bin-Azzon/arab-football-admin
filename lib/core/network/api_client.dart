...
class ApiClient {
  ...
  Uri _uri(String path, [Map<String, dynamic>? query]) {
    final base = Uri.parse(ApiConstants.serverUrl);
  ...
  Future<ApiResponse<T>> multipart<T>(
    ...
    try {
      ...
      throw const ApiException(
        'انتهت مهلة رفع البيانات. تحقق من تشغيل الخادم.',
      );
    } on ApiException {
      rethrow;
    } catch (_) {
      throw const ApiException(
        'تعذر رفع البيانات. تحقق من الاتصال بالخادم.',
      );
    }
  }
  ...
  Future<ApiResponse<T>> _safeRequest<T>(
    ...
    try {
      ...
      throw const ApiException(
        'تعذر الاتصال بالخادم. تحقق من تشغيل الباك اند وعنوان السيرفر.',
      );
    }
  }
  ...
  String _messageFrom(ApiResponse<dynamic> response, int statusCode) {
    ...
    if (statusCode == 400) return 'البيانات المرسلة غير صحيحة.';
